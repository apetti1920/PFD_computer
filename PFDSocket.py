...
class PFDSocket:
        ...
        while self.running.is_set():
            if not self.has_connection.is_set():
                try:
                    ...
                    conn_msg = {'connect': 'PFD'}
                    self.sendmsg(json.dumps(conn_msg))
                    self.has_connection.set()
    ...
    def sendmsg(self, msg: str):
        ...
        """
        send_thread = Thread(target=self._sendmsg, args=(msg,), name='Message Send')
    ...
    def _sendmsg(self, msg: str):
        ...
        """
        try:
            self.sendConn.send(msg.encode('utf8'), )
        except socket.error:
            self.has_connection.clear()
